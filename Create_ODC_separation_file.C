...
// scan_type: 1 => x-scan; 2 => y-scan
{
	...
	{
		if (scan_type == 1) return Get_ODC_x_scan_6012(mid_time);    
		if (scan_type == 2) return Get_ODC_y_scan_6012(mid_time);
		return 0;
	}
	else if (g_vdm_Fill == 6864)
	{
		if (scan_type == 1) return Get_ODC_x_scan_6864(mid_time);    
		if (scan_type == 2) return Get_ODC_y_scan_6864(mid_time);
		return 0;
	}
...
// scan_type: 1 => x-scan; 2 => y-scan
{
	...
	if (scan_type == 1) sprintf(sep_file_name,"../Fill-%d/ODCSep_x_Scan_%d.root", g_vdm_Fill,scan);
	if (scan_type == 2) sprintf(sep_file_name,"../Fill-%d/ODCSep_y_Scan_%d.root", g_vdm_Fill,scan);
