...
void Create_scan_raw_rate_file(Int_t scan_type, Int_t scan, const char *rate_name)
...
// 2.- loop over the range of each step and get the raw rate and its error
{
	...
	char *ddl_counter = new char[kg_string_size];
	sprintf(ddl_counter,"%scounts",rate_name);
	if (g_vdm_Tree->FindBranch(ddl_counter) == NULL)
	{
		cout << " Requested branch ("<< ddl_counter <<") does not exists "  << endl;
		exit(-104);
	}
	...
	g_vdm_Tree->SetBranchAddress(ddl_counter,counter);

	// set up output tree for rates
	char *file_name = new char[kg_string_size];
	if (scan_type == 1) sprintf(file_name,"../Fill-%d/RawRate_%s_x_Scan_%d_allBC.root",g_vdm_Fill,rate_name,scan);
	if (scan_type == 2) sprintf(file_name,"../Fill-%d/RawRate_%s_y_Scan_%d_allBC.root",g_vdm_Fill,rate_name,scan);
	...
	for (Int_t k=0; k<3564; k++) // loop over bunches
	{
		if (k%10 == 0)
		{
			cout <<" Scan " <<scan <<"," <<scan_type <<" working on bunch " <<k <<endl <<flush;
		}
...
void Create_raw_rate_file(Int_t Fill, const char *rate_name)
{
	...
	{
		Create_scan_raw_rate_file(1, i, rate_name);	//x-scans
		Create_scan_raw_rate_file(2, i, rate_name);	//y-scans
	}
