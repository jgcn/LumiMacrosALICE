...
// "+Q", "-Q", "+beta", "-beta", "dQip"
{
  ...
  if (opt == 0) { // chose nominal or ODC separations
    sprintf(bbd_file_name,"../Fill-%d/Corr-%d-sys/Nom-sys/ROOT/bbroot_%d_T0%s.root", 
	    g_vdm_Fill,g_vdm_Fill,g_vdm_Fill,sys_opt);
  } else {
    sprintf(bbd_file_name,"../Fill-%d/Corr-%d-sys/ODC-sys/ROOT/bbroot_%d_T0%s.root",
	    g_vdm_Fill,g_vdm_Fill,g_vdm_Fill,sys_opt);
  }
...
void Create_Run3_BBD_separation_file(Int_t Fill, const char* sys_opt)
// sys_opt = "" default, other options are for systematic studies:
// "+Q", "-Q", "+beta", "-beta", "dQip"
{
  ...
  Get_BBD_separations(0,sys_opt); // NOM separations
  Get_BBD_separations(1,sys_opt); // ODC separations
